...
def rename_tiltmeter_columns(data):
    # Rename columns for Tiltmeter data (TM01 and TM02 for Reading A and B)
    for col in data.columns:
        if 'Reading-A (kSinÎ±)' in col:
            data.rename(columns={col: col.replace('Reading-A (kSinÎ±)', 'Reading-A ()')}, inplace=True)
        elif 'Reading-B (kSinÎ±)' in col:
            data.rename(columns={col: col.replace('Reading-B (kSinÎ±)', 'Reading-B ()')}, inplace=True)
    return data


# Provide the input folder containing CSV files and the output file name
input_folder_tilt = r'C:\Users\khush\Searches\Downloads\workstationgeo'
output_file_tilt = r'C:\Users\khush\Searches\Downloads\workss\test_tilts.csv'
input_folder = r'C:\Users\khush\Searches\Downloads\workstationgeo'
output_file = r'C:\Users\khush\Searches\Downloads\workss\test_cracks.csv'
